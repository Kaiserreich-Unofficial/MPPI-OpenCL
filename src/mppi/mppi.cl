...
float get_obstacle_dist(
    float reaction_time, float go_around, float stop_velocity, float vehicle_length,
    float vehicle_x, float vehicle_y, float vehicle_vel,
    float x, float y, float yaw, float vel, float accel,
    __constant float *waypoints, uint waypoints_size, uint waypoint_dim, uint offset_c_w,
...
void dynamics(float x, float y, float psi, float u, float v, float r,
              float Tl, float Tr, float dx[6]) {
    dx[0] = u * cos(psi) - v * sin(psi);
    dx[1] = u * sin(psi) + v * cos(psi);
    dx[2] = r;
    dx[3] = -0.1189f*u -0.1131f*u*fabs(u)+0.72696f*v*r+1.04965f*Tl+0.72806f*Tr-0.05309f*r*r;
    dx[4] = 0.1577f*v +0.0365f*r -0.7457f*v*fabs(v) -0.0348f*r*fabs(r) -0.22145f*u*v -0.0678f*u*r -0.0899f*Tl +0.1015f*Tr;
    dx[5] = 1.11f*v -0.221f*r -2.712f*v*fabs(v) -0.2083f*r*fabs(r) -0.6508f*u*v -0.6683f*u*r -1.9953f*Tl + 2.1045f*Tr;
}

__kernel void mppi(
    uint N, __constant float *params, __constant float *state,
    ...
    __global float *noise, __global float *costs) {
    ...
    for (uint i = 0; i < N; i++) {
        ...
        dynamics(x+0.5f*delta*k1[0], y+0.5f*delta*k1[1], psi+0.5f*delta*k1[2],
                 u+0.5f*delta*k1[3], v+0.5f*delta*k1[4], r+0.5f*delta*k1[5],
                 Tl, Tr, k2);
        dynamics(x+0.5f*delta*k2[0], y+0.5f*delta*k2[1], psi+0.5f*delta*k2[2],
                 u+0.5f*delta*k2[3], v+0.5f*delta*k2[4], r+0.5f*delta*k2[5],
                 Tl, Tr, k3);
        dynamics(x+delta*k3[0], y+delta*k3[1], psi+delta*k3[2],
                 u+delta*k3[3], v+delta*k3[4], r+delta*k3[5],
                 Tl, Tr, k4);
        ...
        psi = (psi >= 2.0f * M_PI) ? (psi - 2.0f * M_PI) : ((psi < 0.0f) ? (psi + 2.0f * M_PI) : psi);
    ...
    float min_obj_d = get_obstacle_dist(
        reaction_time, go_around, stop_velocity, vehicle_length,
        vehicle_x, vehicle_y, vehicle_u,
        x, y, psi, u, 0.0f,
        waypoints, waypoints_size, waypoint_dim, 0,
        objects, objects_size, object_dim);

    float target_velocity = waypoints[(nb_waypoints-1)*waypoint_dim+3];
    if (u > target_velocity) u = target_velocity;

    float target_safe_dist = safe_dist_coef*u + safe_dist_min;
    float safe_dist = fmax(target_safe_dist - min_obj_d, 0.0f);

    float diff_psi = get_diff_angle(psi, waypoints[(nb_waypoints-1)*waypoint_dim+2]);
    float diff_v = fabs(u - target_velocity);

    float dxt = x - waypoints[(nb_waypoints-1)*waypoint_dim];
    float dyt = y - waypoints[(nb_waypoints-1)*waypoint_dim +1];
    float target_dist = sqrt(dxt*dxt + dyt*dyt);

    cost += 15.0f*min_obj_d*min_obj_d;
    cost += 7.0f*(target_dist > prev_target_dist);
    cost += 120.0f*diff_psi*diff_psi;
    cost += 5.0f*diff_v*diff_v;
