...
MPPI::MPPI(Params params)
{
    ...
    _queue = clCreateCommandQueueWithProperties(_context, _device, props, &ret);
    if (ret != CL_SUCCESS)
    ...
    */
    ...
    for (size_t i = 0; i < _p.N; i++)
    {
        ...
        auto dynamics = [&](float x, float y, float psi, float u, float v, float r,
                            float Tl, float Tr)
        {
            float dx0 = u * std::cos(psi) - v * std::sin(psi);
            float dx1 = u * std::sin(psi) + v * std::cos(psi);
            float dx2 = r;
            float dx3 = -0.1189f * u - 0.1131f * u * std::fabs(u) + 0.72696f * v * r +
                        1.04965f * Tl + 0.72806f * Tr - 0.05309f * r * r;
            float dx4 = 0.1577f * v + 0.0365f * r - 0.7457f * v * std::fabs(v) -
                        0.0348f * r * std::fabs(r) - 0.22145f * u * v -
                        0.0678f * u * r - 0.0899f * Tl + 0.1015f * Tr;
            float dx5 = 1.1100f * v - 0.2210f * r - 2.7120f * v * std::fabs(v) -
                        0.2083f * r * std::fabs(r) - 0.6508f * u * v -
                        0.6683f * u * r - 1.9953f * Tl + 2.1045f * Tr;

            return std::array<float, 6>{dx0, dx1, dx2, dx3, dx4, dx5};
        };

        float dt = _p.delta;

        // k1
        auto k1 = dynamics(x, y, psi, u, v, r, Tl, Tr);

        // k2
        auto k2 = dynamics(x + 0.5f * dt * k1[0],
                           y + 0.5f * dt * k1[1],
                           psi + 0.5f * dt * k1[2],
                           u + 0.5f * dt * k1[3],
                           v + 0.5f * dt * k1[4],
        ...
        // k3
        auto k3 = dynamics(x + 0.5f * dt * k2[0],
                           y + 0.5f * dt * k2[1],
                           psi + 0.5f * dt * k2[2],
                           u + 0.5f * dt * k2[3],
                           v + 0.5f * dt * k2[4],
        ...
        // 更新状态
        x += dt / 6.0f * (k1[0] + 2.0f * k2[0] + 2.0f * k3[0] + k4[0]);
        y += dt / 6.0f * (k1[1] + 2.0f * k2[1] + 2.0f * k3[1] + k4[1]);
        psi += dt / 6.0f * (k1[2] + 2.0f * k2[2] + 2.0f * k3[2] + k4[2]);
        u += dt / 6.0f * (k1[3] + 2.0f * k2[3] + 2.0f * k3[3] + k4[3]);
        v += dt / 6.0f * (k1[4] + 2.0f * k2[4] + 2.0f * k3[4] + k4[4]);
        r += dt / 6.0f * (k1[5] + 2.0f * k2[5] + 2.0f * k3[5] + k4[5]);
        // 三元运算裁剪到 [0, 2*pi]
        psi = (psi >= 2.0f * M_PI) ? (psi - 2.0f * M_PI) : ((psi < 0.0f) ? (psi + 2.0f * M_PI) : psi);

        trajectory.emplace_back(x, y, psi, u, v, r);
